...
### Chance-driven stereo drift

The experiment also adds a shared, slowly moving stereo position before reverb. Each 24–40 second segment has a 65% chance of choosing a target within ±0.22 pan; otherwise it holds the preceding target. Movement uses linear AudioParam ramps, not echoes or audio-rate modulation. The direct voice path and reverb input follow the same motion; the reverb return stays outside the panners. Equal-power center attenuation is compensated for the existing mono voices so centered per-channel level is preserved.

A separate seed-derived random generator controls drift without consuming the musical RNG. Live automation schedules only the next segment ahead and rebases after timer stalls. Export schedules its finite motion in advance. Bus teardown clears the scheduling interval and disconnects both panners and compensation gains. Native browser tests render a steady tone to check center level and changing stereo energy; regression tests check bounds, chance holds, live/export motion agreement, routing, and teardown. This intentionally changes spatial presentation and can change mono downmix level slightly as pan moves; it does not add delay or alter note timing.
