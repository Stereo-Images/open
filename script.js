(() => {
  ...
  // CONSTANTS: AIRIER CATHEDRAL (v132 Settings)
  ...
  // ✅ NEW: Reset all musical/narrative state before each Play
  ...
  function startFromUI() {
    ...
    // Seed + snapshot (unchanged behavior: first Play picks the run; later Plays replay it)
    if (!sessionSnapshot) {
    ...
    // ✅ Critical: reset musical state every time Play is pressed
  ...
  async function renderWavExport() {
    ...
    // ✅ REMOVED: dummyMotif = generateSessionMotif();  (this consumed RNG and broke matching)
    ...
    a.href = url;
    a.download = `open-final-v137-${Date.now()}.wav`;
    document.body.appendChild(a);
    a.click();
