/* ============================================================
   OPEN — v171_true_drift (PLAYER PAGE — separate pages + background stop)
   - Restores v171 behavior
   - Removes popout/hash launch logic (index.html -> player.html via <a>)
   - Fix: Immediate stop when app is backgrounded (visibility/pagehide/blur)
   - AirPlay/iOS: Keep BOTH hidden <video> + hidden <audio> anchors
   - 508: aria-pressed + live region updates (#playerStatus)
   ============================================================ */
...
(() => {
  ...
  function setButtonState(state) {
    ...
    // Visual "ghost button" logic
    if (playBtn) playBtn.classList.toggle("filled", isNowPlaying);
    if (stopBtn) stopBtn.classList.toggle("filled", !isNowPlaying);

    // 508/ARIA pressed state
    if (playBtn) playBtn.setAttribute("aria-pressed", isNowPlaying ? "true" : "false");
    if (stopBtn) stopBtn.setAttribute("aria-pressed", isNowPlaying ? "false" : "true");

    if (toneInput) toneInput.disabled = isNowPlaying;

    setLiveStatus(isNowPlaying ? "Playing" : "Stopped");
  }

  // =========================
  // LIVE RECORDING (optional UI)
  // =========================
  ...
  function startFromUI() {
    ...
    // Ghost downbeat (logic at step 0, audio at step 1)
    phraseCount = -1;
  ...
  function stopAllManual(opts = {}) {
    // opts.reason can be: "manual" | "background" | "pagehide"
    isPlaying = false;
    isEndingNaturally = false;
    isApproachingEnd = false;
    ...
    // If we’re stopping due to backgrounding, suspend ASAP to prevent stutter
    // (This is the key fix for “stop only applies after returning”.)
    if (opts.reason === "background" || opts.reason === "pagehide") {
  ...
  // =========================
  // EXPORT (unchanged hook)
  // =========================
  async function renderWavExport() {
    alert("Export is not wired in this player build unless you add an Export button and keep the full v171 export block.");
  }

  // =========================
  // BACKGROUND / APP SWITCH FIX
  // =========================
  function stopOnBackground(reason) {
    // Only do work if actually playing (prevents “spam”)
    if (!isPlaying) return;
    stopAllManual({ reason });

    // Defensive: if iOS throttles timers, ensure gain hits 0 immediately
    // (setValueAtTime already handles this, but this is a no-op safety.)
    try {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") stopOnBackground("background");
  });

  window.addEventListener("pagehide", () => stopOnBackground("pagehide"));

  // Some iOS/Safari cases fire blur before visibilitychange
  window.addEventListener("blur", () => {
    // Don’t stop if it’s just focus moving within page; blur is “good enough” as early signal.
    stopOnBackground("background");
  });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // Player page should start in stopped state
    setButtonState("stopped");
    ...
    if (stopBtn) stopBtn.addEventListener("click", () => stopAllManual({ reason: "manual" }));
