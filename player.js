/* ============================================================
   OPEN — v51 (Hard Stop + Full Export)
   - Behavior: Hard Stop on background (0% stutter risk).
   - AirPlay: Sticky bridge keeps session alive for control center.
   - Anti-Bleed: Per-run mix bus rebuild.
   - Export: FULL offline rendering logic included.
   ============================================================ */

(() => {
  "use strict";

  const STATE_KEY = "open_player_settings_v51";
  ...
  // Active node tracking (anti-bleed)
  ...
  // AirPlay bridge element (hidden but active)
  function ensureBridge() {
    ...
    Object.assign(bridgeAudioEl.style, {
      ...
      opacity: "0.01", left: "-9999px", top: "0",
      zIndex: "-1", pointerEvents: "none"
    });
    document.body.appendChild(bridgeAudioEl);
  }

  // Build a fresh bus each run (prevents bleed between sessions)
  function buildMixBus() {
    ensureAudioContext();

    // Tear down previous bus route
    if (bus?.masterGain) {
  ...
  function scheduler() {
    ...
    while (nextTimeA < boundary) {
      ...
      if (!(arcPos === 0 && phraseStep < 12 && phraseCount > 0)) {
         scheduleNote(audioContext, bus.masterGain, bus.reverbSend, f, nextTimeA, appliedDur, 0.4, pressure, tension);
      }
  ...
  // =========================
  // HARD STOP ON BACKGROUND
  // =========================
  function handleVisibilityChange() {
    // If hidden, stop immediately. Prevents any chance of stutter.
  ...
  function stopAllManual(instant = false, statusMsg = "Stopped") {
    ...
    // Fade out if manual stop, else cut
    if (!instant && bus?.masterGain && audioContext) {
        ...
        // Instant cut + slight micro-fade to avoid pop
        if (bus?.masterGain && audioContext) {
           bus.masterGain.gain.cancelScheduledValues(audioContext.currentTime);
           bus.masterGain.gain.setTargetAtTime(0, audioContext.currentTime, 0.015);
        }
        setTimeout(() => killAllActiveNodes(), 50);
    }
  ...
  async function renderWavExport() {
    ...
    a.href = url;
    a.download = `open-final-v49-${Date.now()}.wav`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $("stop")?.addEventListener("click", () => stopAllManual(false, "Stopped"));
  ...
  function handleVisibilityOrPageHide() {
    if (isPlaying) stopAllManual(true, "Stopped (background)");
  }
